...
const nav = [
  ...
  { to: "/empleados", label: "Personal", icon: Users },
      ...
      <aside className="flex w-24 flex-col items-center gap-2 border-r border-border bg-sidebar py-4">
        ...
        <nav className="flex flex-1 flex-col items-center gap-1.5">
          {nav.map(({ to, label, icon: Icon }) => {
            const active = to === "/" ? path === "/" : path.startsWith(to.replace(/\/$/, ""));
            return (
              <Link key={to} to={to}
                className={`tap-hi flex w-20 flex-col items-center gap-1 rounded-xl px-2 py-2.5 text-[11px] font-medium ${
                  active ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-surface hover:text-foreground"
                }`}>
        ...
        <div className="text-center text-[9px] leading-tight text-muted-foreground">
          <div className="font-bold">El Sazón</div>
          <div>de Gloria</div>
        </div>
      </aside>
