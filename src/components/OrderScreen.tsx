...
export function OrderScreen({ orderId, mode }: { orderId: string; mode: "dine_in" | "takeaway" }) {
  ...
  const updateQty = useMutation({
    mutationFn: async ({ id, q }: { id: string; q: number }) => {
      if (q <= 0) {
        ...
        await supabase.from("order_items").update({ quantity: q }).eq("id", id);
      }
    },
    onSuccess: invalidate,
  });
  ...
  const askBill = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      toast.success("Mesa marcada como 'esperando cuenta'");
    },
  });
  ...
  const cancelOrder = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      navigate({ to: mode === "dine_in" ? "/" : "/llevar" });
    },
  });
  ...
  if (order.status === "paid" || order.status === "cancelled") {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4 p-8">
        ...
        <button
          onClick={() => navigate({ to: mode === "dine_in" ? "/" : "/llevar" })}
    ...
    <div className="flex h-full">
      ...
      <section className="flex w-[420px] flex-col border-r border-border bg-surface/30">
        <header className="flex items-center justify-between border-b border-border px-5 py-4">
          <button
            onClick={() => navigate({ to: mode === "dine_in" ? "/" : "/llevar" })}
            className="tap-hi flex h-11 w-11 items-center justify-center rounded-xl bg-surface hover:bg-surface-2"
          >
            <ArrowLeft className="h-5 w-5" />
          </button>
          <div className="text-center">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">
              {mode === "dine_in" ? "Mesa" : "Para llevar"}
            </div>
            <div className="text-xl font-bold">
              {mode === "dine_in" ? `Mesa abierta` : order.customer_name || "Sin nombre"}
            </div>
            <div className="text-[11px] text-muted-foreground">{elapsed(order.opened_at)}</div>
          </div>
          <button
            onClick={() => {
              if (confirm("¿Cancelar esta orden? Los items se perderán.")) cancelOrder.mutate();
            }}
                  ...
                  <li key={it.id} className="rounded-2xl bg-surface p-3">
                    <div className="flex items-start gap-2">
                      <div className="flex flex-col items-center gap-1">
                        <button
                          onClick={() => updateQty.mutate({ id: it.id, q: it.quantity + 1 })}
                          className="tap-hi flex h-8 w-8 items-center justify-center rounded-lg bg-surface-2 hover:bg-primary/20"
                        >
                        ...
                        <button
                          onClick={() => updateQty.mutate({ id: it.id, q: it.quantity - 1 })}
                          className="tap-hi flex h-8 w-8 items-center justify-center rounded-lg bg-surface-2 hover:bg-destructive/20"
                        >
                      ...
                      <div className="min-w-0 flex-1">
                          ...
                          <ul className="mt-1 space-y-0.5">
                            {it.order_item_extras.map((e) => (
                              <li key={e.id} className="flex items-center justify-between text-xs">
                                <button
                                  onClick={() => removeExtra.mutate(e.id)}
                        ...
                        {it.notes && (
                          <div className="mt-1 rounded-md bg-warning/15 px-2 py-1 text-[11px] text-warning">
                            📝 {it.notes}
                          </div>
                        )}
                        <div className="mt-2 flex gap-1">
                          ...
                          <button
                            ...
                            className="tap-hi ml-auto flex items-center gap-1 rounded-lg bg-surface-2 px-2 py-1 text-[11px] text-destructive hover:bg-destructive/20"
                          >
        ...
        <footer className="border-t border-border p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="tap-hi flex items-center justify-center gap-2 rounded-xl bg-warning px-4 py-4 font-semibold text-warning-foreground disabled:opacity-40"
            >
        ...
        <div className="grid flex-1 auto-rows-min grid-cols-2 gap-3 overflow-y-auto p-5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {filtered.map((p) => (
            <button
              ...
              className="tap-hi group flex aspect-square flex-col items-center justify-between rounded-2xl bg-surface p-4 text-center hover:bg-surface-2 hover:ring-2 hover:ring-primary"
            >
        ...
        <PayModal
          ...
          onPay={async ({ method, received }) => {
            ...
            if (order.table_id) {
              await supabase
                .from("tables")
                .update({ status: "free", opened_at: null, guests: 0 })
        ...
        <div className="grid grid-cols-2 gap-2">
          {extras.map((e) => (
            <button
              ...
              className="tap-hi flex flex-col items-start rounded-xl bg-surface p-3 text-left hover:bg-primary/20"
            >
...
function PayModal({
  ...
  total: number;
  onClose: () => void;
  onPay: (p: { method: string; received: number }) => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="mb-4 grid grid-cols-3 gap-2">
          {[
            ...
            <button
              ...
              className={`tap-hi rounded-xl py-4 text-sm font-semibold ${
                method === m.v ? "bg-primary text-primary-foreground" : "bg-surface hover:bg-surface-2"
              }`}
            >
              {m.l}
            </button>
        ...
        <button
          onClick={() => onPay({ method, received: Number(received || total) })}
          className="tap-hi w-full rounded-xl bg-primary py-5 text-lg font-bold text-primary-foreground"
        >
          Confirmar pago
        </button>
      </div>
